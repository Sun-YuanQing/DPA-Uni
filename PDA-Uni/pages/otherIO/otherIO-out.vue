...
<template>
	<view>
		<view id="viewHeader">
			<!-- 头表信息 -->
			<barcode-input ref="input_order" @onScaned="onOrderScaned" focus="true" placeholder="扫描出库单号"></barcode-input>
					 ...
					 @click="onSelectedItem(i)" v-bind:class="item.selectItemClass">
						<view class="uni-triplex-row">
							...
							<view class="uni-triplex-right">
								<text class="uni-h5" style="min-width: 200upx;margin-left: -300upx;">批次:{{ item.inLot?item.inLot:''}}</text>
	...
	export default {
		...
		methods: {
			...
			init(isinit) {
				...
				apis.Pda_scmotheroutbysheetnopartno({
					data: {
			...
			onSelectedItem(e) {
				...
				uni.navigateTo({
					url: 'otherIO-outDet?progNo=' + _self.progNo
				});
			}
		}
	};
