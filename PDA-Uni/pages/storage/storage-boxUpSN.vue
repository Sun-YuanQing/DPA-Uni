...
<template>
	<view>
		<view id="viewHeader">
			...
			<view>
				<view class="uni-list-cell input-row">
					...
					<view class="uni-list-cell-db">{{ orderHeader.partItemNo }}</view>
				</view>
				<view class="uni-list-cell input-row">
					<view class="uni-list-cell-left">物料名称:</view>
					<view class="uni-list-cell-db">{{ orderHeader.partName }}</view>
				</view>
				<view class="uni-list-cell input-row">
					<view class="uni-list-cell-left">每箱箱数:</view>
					<view class="uni-list-cell-db">{{ orderHeader.boxPackingQty }}</view>
					<view class="uni-list-cell-left">已扫描数量:</view>
					<view class="uni-list-cell-db">{{ orderHeader.totalQty }}</view>
				</view>
			</view>
		</view>
		<!-- 显示入库单明显 -->
		...
		<!-- 底部按钮模块 -->
		<page-foot ref="viewBtm"><print-button ref="printBtn" buttonText="封箱" :reportModel="reportModel" @click="onSave" @tagValid="onBoxScanSave"></print-button></page-foot>
	</view>
</template>
...
export default {
	...
	methods: {
		onPartScaned(e) {
			...
			apis.Pda_pdaserialinboxvaild({
				...
				success: res => {
					...
					if (this.orderHeader.poMoSoLot != res.poMoSoLot) {
						this.orderHeader.poMoSoLot = '';
					}
		...
		onBoxScanSave(scanBoxTagModelList) {
			...
			/* 与原 装箱 的区别 */
			scanBoxTagModel.binWhNo = this.details[0].binWhNo;
			scanBoxTagModel.binNo = this.details[0].binWhNo;
