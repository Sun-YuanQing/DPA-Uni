		...
		<view class="uni-list-cell input-row">
			...
			<view>
				...
				<view class="uni-list-cell input-row">
					...
					<view class="uni-list-cell-db">{{ order.outSpcQty }}</view>
					<view class="uni-list-cell-left">送检数量:</view>
					...
					<view class="uni-list-cell scroll-view-item" hover-class="uni-list-cell-hover" @click="onSelectedItem(index)" v-bind:class="item.selectItemClass">
						<view class="uni-triplex-row">
							...
							<view class="uni-triplex-right">
								...
								<text class="uni-h5" style="margin-left:-200px ;">已检标签数:{{ item.scanQty }}</text>
								<text class="uni-h5" style="margin-left:-200px ;">未检标签数:{{ item.lastScanQty }}</text>
							</view>
						</view>
					</view>
				</block>
...
export default {
	...
	onLoad(options) {
		...
		//console.log(util.outputLog(this.order));
	},
	onReady() {
	...
	methods: {
		...
		if (Number(this.sheetQty) == 0) {
			...
			apis.boxfqc_boxScan({
				...
				success: res => {
					...
					if (Number(this.sheetQty) > Number(this.order.outSpcQty)) {
						uni.showToast({
					...
					var _self = this;
					/* 销售通知单仓库对比外箱仓库 */
					console.log(this.order.whNo + '---' + res.binWhNo);
					if (this.order.whNo != res.binWhNo) {
						uni.showToast({
							title: '销售通知单仓库：' + this.order.whNo + '与外箱仓库:' + res.binWhNo + '不一致',
							icon: 'none'
						});
						util.showAudio();
						this.$refs.input_box.setFocus();
							...
							uni.showModal({
								...
								success: function(res_mod) {
									if (res_mod.confirm) {
										...
										_self.testTotal = util.floatObj.subtract(_self.testTotal, res.sheetQty);
					...
					res.lastScanQty = res.tagDetails.length;
					_self.boxlist.push(res);
					_self.testTotal = util.floatObj.add(_self.testTotal, res.sheetQty);
		...
		onPartScaned(e) {
			...
			apis.boxfqc_QRCodeAnalysis({
				...
				success: res => {
					...
					for (let i = 0; i < _self.boxlist.length; i++) {
						let item = _self.boxlist[i];
						for (let j = 0; j < item.tagDetails.length; j++) {
							let tagItem = item.tagDetails[j];
					...
					if (pos.x == -1) {
						...
						_self.boxlist[pos.x].scanQty = util.floatObj.add(_self.boxlist[pos.x].scanQty, 1);
						_self.boxlist[pos.x].lastScanQty = util.floatObj.subtract(_self.boxlist[0].tagDetails.length, _self.boxlist[pos.x].scanQty);
					}
				...
				//util.automateSave(_self.boxlist, 'lastScanQty', 0, _self.onSave);
				},
				failure: message => {
					util.showAudio();
		...
		onSave(e) {
			...
			for (let i = 0; i < this.boxlist.length; i++) {
				...
				clone.purLot = clone.poMoSoLot;  //oqc没有这个值
