...
var jpPrinter = {
	createNew: function() {
		...
		jpPrinter.addText = function(x, y, text, font, x_, y_) { //打印文字
			data = "TEXT " + x + "," + y + ",\"" + font + "\"," + 0 + "," + x_ + "," + y_ + "," + "\"" + text + "\"\r\n"
			jpPrinter.addStrToCommand(data)
		};

		jpPrinter.addBarcode = function(x, y, content, codetype, height, readable, portrait, radio) { //打印条形码
			let rotation = portrait ? 0 : 90;
			let narrow, wide
			narrow = wide = radio;
		...
		jpPrinter.addQR = function(x, y, content, width) { //打印二维码
			data = "QRCODE " + x + "," + y + ", M," + width + ", A," + 0 + ",\"" + content + "\"\r\n"
			jpPrinter.addStrToCommand(data)
		};
