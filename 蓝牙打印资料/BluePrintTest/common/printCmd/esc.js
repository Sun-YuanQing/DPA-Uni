...
var jpPrinter = {
	createNew: function() {
		...
		jpPrinter.addText = function(content) { //设置文本内容
		...
		// jpPrinter.addBarcodeWidth = function(width) { //设置条码宽度
		//   data.push(29)
		//   data.push(119)
		//   if (width > 6) {
		//     width = 6;
		...
		// jpPrinter.addBarcodeHeight = function(height) { //设置条码高度
		//   data.push(29)
		//   data.push(104)
		//   data.push(height)
		// }

		// jpPrinter.addBarcodeContent = function(t,content) {
		//   var ty = 73;
		//   data.push(29)
		//   data.push(107)
		//   switch (t) {
		...
		jpPrinter.addSelectSizeOfModuleForQRCode = function(n) { //设置二维码大小
			data.push(29)
			data.push(40)
			data.push(107)
			data.push(3)
		...
		jpPrinter.addSelectErrorCorrectionLevelForQRCode = function(n) { //设置纠错等级
			/*
			n      功能      纠错能力
			48 选择纠错等级 L 7
			49 选择纠错等级 M 15
		...
		jpPrinter.addStoreQRCodeData = function(content) { //设置二维码内容
			var code = new encode.TextEncoder(
				'gb18030', {
					NONSTANDARD_allowLegacyEncoding: true
				}).encode(content)
		...
		jpPrinter.addPrintQRCode = function() { //打印二维码
			data.push(29)
			data.push(40)
			data.push(107)
			data.push(3)
		...
		jpPrinter.addHorTab = function() { //移动打印位置到下一个水平定位点的位置
			data.push(9)
		}

		jpPrinter.addAbsolutePrintPosition = function(where) { //设置绝对打印位置
			data.push(27)
			data.push(36)
		...
		jpPrinter.addRelativePrintPositon = function(where) { //设置相对横向打印位置
			data.push(27)
			data.push(92)
			data.push(parseInt(where % 256))
			data.push(parseInt(where / 256))
		...
		jpPrinter.addSelectJustification = function(which) { //对齐方式
			/*
			0, 48 左对齐
			1, 49 中间对齐
			2, 50 右对齐
		...
		jpPrinter.addLeftMargin = function(n) { //设置左边距
			data.push(29)
			data.push(76)
		...
		jpPrinter.addPrintingAreaWidth = function(width) { //设置打印区域宽度
			data.push(29)
			data.push(87)
		...
		jpPrinter.addSound = function(n, t) { //设置蜂鸣器
			data.push(27)
			data.push(66)
			if (n < 0) {
				n = 1;
		...
		jpPrinter.addBitmap = function(res) { //参数，画布的参数
			console.log(res)
		...
		jpPrinter.addPrint = function() { //打印并换行
			data.push(10)
		}

		jpPrinter.addPrintAndFeed = function(feed) { //打印并走纸feed个单位
			data.push(27)
			data.push(74)
			data.push(feed)
		}

		jpPrinter.addPrintAndFeedRow = function(row) { //打印并走纸row行
			data.push(27)
			data.push(100)
			data.push(row)
		}
