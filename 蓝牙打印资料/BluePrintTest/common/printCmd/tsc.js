...
var jpPrinter = {
	createNew: function() {
		...
		jpPrinter.addCashdrwer(m, t1, t2) { //该指令用于产生钱箱控制脉冲
		...
		jpPrinter.addBar = function(x, y, width, height) { //绘制线条
			data = "BAR " + x + "," + y + "," + width + "," + height + "\r\n"
			jpPrinter.addStrToCommand(data)
		};

		jpPrinter.addText = function(x, y, font, x_, y_, str) { //打印文字
			data = "TEXT " + x + "," + y + ",\"" + font + "\"," + 0 + "," + x_ + "," + y_ + "," + "\"" + str + "\"\r\n"
			jpPrinter.addStrToCommand(data)
		};

		jpPrinter.add1DBarcode = function(x, y, codetype, height, readable, narrow, wide, content) { //打印条形码
		...
		jpPrinter.addQR = function(x, y, level, width, mode, content) { //打印二维码
		...
		jpPrinter.addQueryPrinterType() { //该指令回复打印机的型号
			data = "~!T\r\n";
			jpPrinter.addStrToCommand(str);
		}
		jpPrinter.addQueryPrinterStatus() { //询问打印机状态指令
			data = "<ESC>!?\r\n";
			jpPrinter.addStrToCommand(str);
		}
		jpPrinter.addResetPrinter() { //该指令命令打印机重新开机
			data = "<ESC>!R\r\n";
			jpPrinter.addStrToCommand(str);
		}

		jpPrinter.addQueryPrinterLife() { //该指令回复打印机已打印的里程
			data = "~!@\r\n";
			jpPrinter.addStrToCommand(str);
		}

		jpPrinter.addQueryPrinterMemory() {
			data = "~!A\r\n";
			jpPrinter.addStrToCommand(str);
		}

		jpPrinter.addQueryPrinterFile() {
			data = "~!F\r\n";
			jpPrinter.addStrToCommand(str);
		}

		jpPrinter.addQueryPrinterCodePage() {
			data = "~!I\r\n";
			jpPrinter.addStrToCommand(str);
		}

		jpPrinter.addPeel(enable) { //该指令用来启动/关闭剥离模式，默认值为关闭
		...
		jpPrinter.addTear(enable) {
			data = "SET TEAR " + enable + "\r\n";
			jpPrinter.addStrToCommand(str);
		}

		jpPrinter.addCutter(enable) { //此命令用于设置切刀状态，关闭打印机电源后，该设置将会被存储在打印机内存中。
			data = "SET CUTTER " + enable + "\r\n";
			jpPrinter.addStrToCommand(str);
		}

		jpPrinter.addCutterBatch() {
			data = "SET CUTTER BATCH\r\n";
			jpPrinter.addStrToCommand(str);
		}

		jpPrinter.addReprint(enable) { //此命令将禁用/启用标签机在无纸或开盖错误发生后，上纸或合盖后重新打印一
			次标签内容
			data = "SET REPRINT " + enable.getValue() + "\r\n";
			jpPrinter.addStrToCommand(str);
		}

		jpPrinter.addPrintKey(enable) { //此命令将打印一个标签并走到下一个标签的间隙到撕纸位置处，按下 FEED 按键，打印下一个标签或多份的标签。如果标签内容包括串行文字或条形码，它将改变序号，此设置将保存在打印机内存
		...
		jpPrinter.addUserCommand(command) {//添加自定义命令
