...
var jpPrinter = {
	createNew: function() {
		...
		jpPrinter.setGap = function(printGap) { //该指令用于定义两张卷标纸间的垂直间距距离
		...
		jpPrinter.setTear = function(enable) { // ON/OFF 此命令是用来启用/禁用撕纸位置走到撕纸处，此设置关掉电源后将保存在打印机内
			data = "SET TEAR " + enable + "\r\n";
			jpPrinter.addCommand(data)
		};

		jpPrinter.setQueryPrinterStatus = function(mode) { //ON/OFF/BATCH 此指令用于设置打印机自动返回状态
